...
function showPredictionResult(response) {
  ...
  result.innerHTML = `
    ...
    <div class="prediction-result">
      ...
      <div>
        <div class="result-title">Nutrition:</div>
        <div>${data.nutrition}</div>
      </div>
    </div>
  `;
}
...
function showErrorMessage(error) {
  result.innerHTML = `
    <div class="response-message error">
      ...
      <span class="message">Gambar tidak dapat diprediksi, Silahkan refresh page dan pilih gambar lain</span> <br>
      <img style="width: 100px;" src="src/images/error.svg" alt="Error Image" />
    </div>
  `;

  showElement(result); // Ensure the result container is visible
}
